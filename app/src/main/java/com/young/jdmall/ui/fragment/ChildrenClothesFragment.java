...
public class ChildrenClothesFragment extends CategoryBaseRightListFragment {
    ...
    public void loadNetwork() {
        ...
        if (mData != null) {
            ...
            for (int i = 0; i < mData.getCategory().size(); i++) {
                ...
                //找到目标ID,只有第一个时执行
                if (categoryBean.getId() == 23) {
                    //设置标题
                    CategoryBaseBean categoryInfoBean = new CategoryBaseBean();
                    categoryInfoBean.setTitle("儿童新款");
                    //添加内容
                ...
                } else if (categoryBean.getId() == 125) {
                    //设置标题
                    CategoryBaseBean categoryInfoBean = new CategoryBaseBean();
                    categoryInfoBean.setTitle("儿童时尚");
                    //添加内容
                    ...
                    //设置标题
                    CategoryBaseBean categoryInfoBean = new CategoryBaseBean();
                    categoryInfoBean.setTitle("儿童服饰");
                    //添加内容
                    List<CategoryBaseBean.CategoryBean> categoryBeen = new CopyOnWriteArrayList<>();
                    //在一的里面再次循环,判断是否是需要的ID,如果是,那么添加进去
                    for (int j = 0; j < mData.getCategory().size(); j++) {
